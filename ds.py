...
def get_train_val(data_df, fea_start, english_material_all_df, transforms=None):
    ...
    for i in tqdm.tqdm(range(data_df.shape[0])):
        if word2s[data_df['WORD_ID'][i]] != sentence_id:
    ...
    train_dataset = dataset(data_df, fea_start, train_sen_data, transforms)
    val_dataset = dataset(data_df, fea_start, val_sen_data, transforms)
    return train_dataset, val_dataset

class dataset(data.Dataset):
    def __init__(self, data_df, fea_start, sen_data, transforms=None):
        '''
    ...
    def __getitem__(self, item):
        ...
        label = self.data_df.iloc[self.sen_data[item][0]:self.sen_data[item][0]+self.sen_data[item][1],
                                    0].values
        label = np.concatenate((label, np.zeros(self.max_length-data.shape)))
