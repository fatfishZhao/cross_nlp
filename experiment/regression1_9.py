...
import numpy as np
def load_data(fea_path = '../data/EGCO/al_feature.pkl'):
    with open(fea_path, 'rb') as f:
        fea_df = pd.DataFrame(pickle.load(f))
    fea_df = fea_df[['WORD_TOTAL_READING_TIME','WORD']+list(fea_df.columns)[-11:]]

    fea_df = fea_df.replace('.', 0)
    fea_df = fea_df[fea_df['WORD_TOTAL_READING_TIME']!=0]
    fea_df = fea_df.dropna()
    data_Y = fea_df.iloc[:,0].values
    data_X = fea_df.iloc[:,2:].values
    return data_X, data_Y
data_X, data_Y = load_data('../data/EGCO/all_feature.pkl')
